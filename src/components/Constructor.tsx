...
const Constructor: FC<ConstructorProps> = ({ children }) => {
  const dragSensor = useAppSelector((state) => state.constructorSlice.items);

  return (
    <div className={`sidebar-constuctor__content`}>
      {children}
      {!dragSensor.some((obj) => obj.draggable === false) && (
        <div className="sidebar-constructor__placeholder">
          <svg
            width="22"
            height="22"
            viewBox="0 0 22 22"
          ...
          <div className="sidebar-constuctor__blue">Перетащите сюда</div>
          <div className="sidebar-constuctor__bottom">
            любой элемент
            <br /> из левой панели
          </div>
        </div>
